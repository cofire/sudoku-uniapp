...
class Board {
    ...
    for (let i = 1; i < 9; i += 1) {
      for (let j = 0; j < 9; j += 1) {
        ...
        if (grid.choice === undefined) {
          used = this.getUsedValueArrayAt({
            row: i,
            col: j
          });
          unused = Utils.getRandomValue({
            exclude: used
          });
  ...
  getRandomValidValue(pos) {
    ...
    let valueArray = Utils.getRandomValue({
      exclude: used
    });
  ...
  getUsedValueArrayAt(pos) {
    ...
    let rowGrids = this.getUsedValueArray({
      mode: "row",
      row: row
    });
    let colGrids = this.getUsedValueArray({
      mode: "column",
      col: col
    });
    let blockGrids = this.getUsedValueArray({
      mode: "block",
      row: block.row,
      col: block.col
    });
...
class SudokuGame {
  constructor(difficuty) {
    ...
    this.digBoard();
    this.sudoku = Utils.gridsTosudoku(this.board);
  }
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        for (let k = 0; k < this.digTimes; k++) {
          ...
          block[dig].setValue(0);
          block[dig].setType("new");
        }
      }
    }
...
SudokuGame.DifficutyEasyer = 2;
SudokuGame.DifficutyEasy = 3;
SudokuGame.DifficutyNormal = 4;
SudokuGame.DifficutyHard = 5;
